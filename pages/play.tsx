...
export default function PlayPage() {
  ...
  function copyURL() {
    ...
    navigator.clipboard.writeText(
      `https://customwordle.vercel.app${router.asPath}`
    )
    focusField()
    setTimeout(function () {
      setShowCopy(false)
  ...
  return (
    ...
    flex flex-col justify-between min-h-screen">
      ...
      <main className="mx-auto w-full max-w-2xl pb-12">
        <h1 className="my-6 text-center underline text-3xl font-bold text-slate-100">
          Play Custom Wordle!
        </h1>
        ...
        />
          ...
          <span className="mr-2 font-medium">Share with a friend:</span>
          <input
            className="rounded border border-slate-800 bg-slate-700 px-2 py-1"
            readOnly
            value={`https://customwordle.vercel.app${router.asPath}`}
          />
      ...
      <footer className="flex h-15 w-full items-center justify-center border-t">
        <a
